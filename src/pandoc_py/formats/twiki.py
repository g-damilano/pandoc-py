...
def _read_twiki(source: str) -> Document:
    """TWiki reader. Heading: ``---+`` (level 1), ``---++`` (level 2), …"""
    text = source.replace('\r\n', '\n').replace('\r', '\n')
    lines = text.split('\n')
    blocks = []
    i = 0
    while i < len(lines):
        ...
        if line.lstrip().startswith('<verbatim>'):
            buf, j = [], i + 1
            while j < len(lines) and not lines[j].lstrip().startswith('</verbatim>'):
        ...
        while j < len(lines) and lines[j].strip() and not (
            ...
            lines[j].lstrip().startswith('<verbatim>')
        ):
            buf.append(lines[j]); j += 1
        blocks.append(Paragraph(inlines=text_to_inlines(' '.join(s.rstrip() for s in buf))))
        i = j
    return Document(blocks=blocks, source_format='twiki')


def _write_twiki(document: Document) -> str:
    out: list[str] = []
    for block in document.blocks:
        if isinstance(block, Heading):
            ...
            out.append('<verbatim>'); out.extend(block.text.split('\n')); out.append('</verbatim>'); out.append('')
        elif isinstance(block, ThematicBreak):
            out.append('---'); out.append('')
    return '\n'.join(out).rstrip('\n') + '\n'


class TwikiReader(Reader):
    format_name = 'twiki'
    def read(self, source, options=None):
        if isinstance(source, bytes): source = source.decode('utf-8')
        return _read_twiki(source)

class TwikiWriter(Writer):
    format_name = 'twiki'
    def write(self, document, options=None):
        return _write_twiki(document)

register_reader(TwikiReader())
register_writer(TwikiWriter())
