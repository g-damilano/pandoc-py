...
def _read_txt2tags(source: str) -> Document:
    """t2t reader: ``= text =`` H1, ``== text ==`` H2, etc."""
    text = source.replace('\r\n', '\n').replace('\r', '\n')
    lines = text.split('\n')
    blocks = []
    i = 0
    while i < len(lines):
        ...
        i = j
    return Document(blocks=blocks, source_format='txt2tags')


def _write_txt2tags(document: Document) -> str:
    out: list[str] = []
    for block in document.blocks:
        if isinstance(block, Heading):
            level = max(1, min(block.level, 5))
            text = inlines_to_plain(block.inlines)
            out.append('=' * level + ' ' + text + ' ' + '=' * level)
            out.append('')
...
class Txt2TagsReader(Reader):
    format_name = 'txt2tags'
    aliases = ('t2t',)
    def read(self, source, options=None):
        if isinstance(source, bytes): source = source.decode('utf-8')
        return _read_txt2tags(source)

class Txt2TagsWriter(Writer):
    format_name = 'txt2tags'
    aliases = ('t2t',)
    def write(self, document, options=None):
        return _write_txt2tags(document)

register_reader(Txt2TagsReader())
register_writer(Txt2TagsWriter())
