...
- Implemented-or-better rows: 501/589 (85.1%).
...
- v55 native-wrapper implementation tranche: native reader now accepts top-level `Pandoc` wrappers with metadata decoding (`nullMeta`, `Meta { unMeta = fromList ... }`, and `Meta*` values), but oracle differential admission is still pending because the current environment does not provide `/usr/bin/pandoc`.
...
- Package version: pandoc_py 0.0.55.
